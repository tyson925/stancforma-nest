...
//            System.out.println(" not infi ");
    ...
    public static NestPath linearize2(Segment p1, double rx, double ry, double r) {
        int n = 5;
        NestPath finished = new NestPath();


        double x = p1.x + r * Math.cos(rx);
        double y = p1.y + r * Math.sin(rx);
        finished.add(new Segment(x, y));
        x = p1.x + r * Math.cos(ry);
        y = p1.y + r * Math.sin(ry);
        finished.add(new Segment(x, y));
    ...
    public static NestPath linearize(Segment p1, double rx, double ry, double r) {
        int n = 5;
        NestPath finished = new NestPath();

        for (int i = 0; i < n; i++) {
            double t = Math.abs(rx - ry) * i / n;
            double x = p1.x + r * Math.cos((rx + t));
            double y = p1.y + r * Math.sin((rx + t));
    ...
    public static NestPath linearize(Segment p1, double rx, double ry, double angle, int laregearc, int sweep, double tol) {
        NestPath finished = new NestPath();
        //finished.add(p2);
        //DataExchange arc = ConvertToCenter(p1,p2,rx,ry,angle,laregearc,sweep);
        DataExchange arc = new DataExchange(new Segment(p1.x, p1.y), rx, ry, 0.0, radiansToDegree(angle), angle, false);
